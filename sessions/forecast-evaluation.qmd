...
So far we have focused on building and visualising simple forecasts.
...
-   [Forecast evaluation](slides/forecast-evaluation.qmd)
...
In this session we will use the `fable` package for fitting and evaluating simple forecasting models, the `dplyr` package for data wrangling, the `ggplot2` and `ggtime` packages for plotting and the `epidatr` package for accessing and downloading versions of epidemiological surveillance data from the [Delphi EpiData API](https://cmu-delphi.github.io/delphi-epidata/).
...
For the first time, we have confronted a model with the eventually observed data.
...
-   In general, the trend of the forecasts for the first few horizons match the recent trend in the observed data.
-   The model seems to do fairly well at the tails of the season. This is likely because at the "shoulders" of the season the patterns are more consistent and don't vary as much from one season to the other.
-   The model's predictions for the epidemiologically important part (the peak!) are not very good - it's much too low. The observed data are outside the 95% PI intervals for many weeks in a row.
...
By balancing these two aspects, the CRPS provides an overall measure of the quality of probabilistic forecasts.
...
The image below shows the experimental set-up.
...
For this experiment, let's define five models:
...
2.  the ARIMA(2,0,0), also sometimes just shortened into AR(2), model that we fit earlier.
3.  a seasonal baseline model that just has fourier terms and no ARIMA terms.
4.  a model that has both the AR(2) and fourier terms.
5.  a **copycat** model: a probabilistic *method of analogues*. Rather than extrapolating a parametric time-series model, it matches the current season's recent growth pattern against a library of past influenza seasons and reuses the subsequent growth of the closest analogues, re-anchored to the most recent observed value.

::: callout-tip
The `COPYCAT()` model and its analogue library `copycat_db` come with the course package. The library holds every US influenza season before 2015/16 (National, the HHS regions, and the states), drawn from ILINet. It stores each season as a **log weekly growth rate** by week-of-season, which is scale-free — so seasons from locations of very different sizes are directly comparable, and the model can forecast the national wILI series directly (no transformation needed). It is a reimplementation of the [copycat model](https://github.com/sjfox/copycat) by Spencer Fox / epiENGAGE. The copycat model is similar in spirit to other method of analogue implementations for infectious disease forecasting.[@viboud_prediction_2003;@ray_infectious_2017]
:::
...
Sorted by CRPS, the `copycat` analogue model scores best: its *probabilistic* forecasts — which is what CRPS rewards — are the most accurate overall, by a clear margin.
Notice, though, that `fourier_ar2` has the lowest **MAE** and **RMSE**: its *point* forecasts (the median) are closest on average, even though its prediction *intervals* are less well calibrated than copycat's.
This is a good reminder that point accuracy and probabilistic accuracy can rank models differently — a model can have a well-placed central estimate but poorly-calibrated uncertainty, or vice versa.
...
-   At larger horizons, there is a clearer distinction between the models. Among the ARIMA-type models, `fourier` and `fourier_ar2` are lower than the others, with `fourier_ar2` the best of the parametric models.
-   The `copycat` analogue model, however, has the lowest CRPS at **every** horizon, and its advantage grows with horizon. Its performance also varies quite a bit by forecast date, so it appears that it was able to predict a few key moments (the steep rise) very well, while not predicting the initial decline as well as other models.
...
-   Projecting a flat line from the data (what the random walk does, essentially) does not do well at the important and dynamic times of the season.
    Among the parametric models, the combination of factoring in some auto-regressive and seasonal dynamics does best; the analogue (`copycat`) approach, which borrows whole-season shapes rather than extrapolating, does better still on CRPS.
:::
