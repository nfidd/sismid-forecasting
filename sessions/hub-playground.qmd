...
In this session we will use the `dplyr` package for data wrangling, the `ggplot2` and `ggtime` packages for plotting (`ggtime` provides the time-series and forecast plotting functions that were previously part of `fabletools`/`feasts`), the `fable` package for building forecasting models, and the `epidatr` package for downloading epidemiological surveillance data.
