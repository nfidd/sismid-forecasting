...
-   [Introduction to forecasting](slides/introduction-to-forecasting.qmd)
...
In this class, we will largely focus on settings where you have multiple prior "seasons" of data to model.
Especially when using a mechanistic model that makes assumptions about the biological mechanisms of disease transmissions, is can also be possible to generate forecasts for emerging outbreaks. Although great care is needed to ensure that appropriate validation is being performed in these settings.
...
We will look at this in more detail later in the course, but it is worth seeing now what format `fable` uses to store the forecast itself.
