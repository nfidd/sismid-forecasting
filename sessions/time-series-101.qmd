...
The course package provides a fourth-root transformation, `my_fourth_root`, defined with `fable`'s `new_transformation()` standard from `fourth_root(x) = x^0.25` and its inverse `inv_fourth_root(x) = x^4`. We use it throughout the course.
