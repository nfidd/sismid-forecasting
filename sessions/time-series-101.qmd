...
- [Time series forecasting 101](slides/time-series-101.qmd)
...
The source file of this session is located at `sessions/time-series-101.qmd`.

## Libraries used

In this session we will use `nfidd` package for some flu data, the `fable` package to fit and forecast ARIMA models, the `dplyr` and `tidyr` packages for data wrangling, the `ggplot2` and `ggtime` packages for plotting, and the `epidatr` package for loading epidemiological data.
...
We will now use the function `gg_tsdisplay()` from [the `ggtime` R package](https://pkg.mitchelloharawild.com/ggtime/) to give a unique view into our dataset.

```{r}
ggtime::gg_tsdisplay(flu_data, y = wili, plot_type='partial', lag_max = 104)
```
...
We are not going to give an in-depth review of ARIMA models in this course, but we encourage you to check out the [FPP3 Chapter on ARIMA models](https://otexts.com/fpp3/arima.html), which provides a very accessible introduction to the concepts and math behind this modeling framework.
...
**Note the constant variance assumption!** This is one of the places where the stationarity assumption is comes into play: if the time-series is stationary then constant variance is a reasonable assumption.
...
```{r, warning=FALSE}
ggtime::gg_tsdisplay(flu_data, y = difference(wili), plot_type='partial', lag_max = 104)
```
...
The course package provides a fourth-root transformation, `my_fourth_root`, defined with `fable`'s `new_transformation()` standard.
We use it throughout the course.
This is the code for the transformation:
...
- Read about [ARIMA models in the FPP3 textbook](https://otexts.com/fpp3/arima.html). Try to develop a better understanding of how $p$, $d$, and $q$ impact the behavior of a time-series.
