...
This value matches the value in the ACF plot (lower left, above) at x=1:
```{r}
cor(flu_data$wili, dplyr::lag(flu_data$wili, 1), use = "complete.obs")
```

And this value matches the value in the ACF plot (lower left, above) at x=12, just after the values switch from being positive to negative:
```{r}
...
The course package provides a fourth-root transformation, `my_fourth_root`, defined with `fable`'s `new_transformation()` standard. We use it throughout the course. This is the code for the transformation:
