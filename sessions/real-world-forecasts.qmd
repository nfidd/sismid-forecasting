...
So far in this course we have focused on building, visualising and evaluating "toy" forecast models in somewhat synthetic settings.
...
-   [Real-world forecasts](slides/introduction-to-real-world-forecasts.qmd)
...
The project collected nearly 1 billion rows of model output data, and has led to multiple research papers on epidemiological forecasting.[@ray_comparing_2023; @cramer_evaluation_2022; @fox_optimizing_2024; @kim_beyond_2026]
...
However, there are over 7 million rows of data, and it can take some bandwidth to download.
...
You could review the [Baselines and Seasonality session](baselines-and-seasonality.qmd) for a refresher on different types of baselines.
:::
...
Recall that these are the "ground truth" (or target) data, formatted like they were being predicted by a future-seeing model.
Here is code to pull oracle-output data from the hub itself, and the first few rows of the table.
...
-   Tying back to renewal-equation approaches (covered in the companion [Nowcasting & $R_t$ estimation course](https://nfidd.github.io/sismid-nowcasting/sessions/R-estimation-and-the-renewal-equation.html)):
...
While the ensemble remains the most accurate model, now five models are better than the baseline, and the `UMass-gbqr` model is the most accurate individual model (just barely edging out the `CMU-TimeSeries` and `UMass-ar6_pooled` models.)
...
As we discussed in the [session on evaluating forecasts](forecast-evaluation.qmd), overall comparisons can miss details about ways that models perform across different forecast dimensions.
...
- One of the very first papers comparing forecasts from the CDC FluSight challenge.[@biggerstaff_results_2016]
- A paper describing an open forecasting challenge for dengue fever. [@johansson_open_2019]
- Papers describing forecasts for cases [@lopezChallengesCOVID19Case2024] and deaths [@cramer_evaluation_2022] in the COVID-19 pandemic.
