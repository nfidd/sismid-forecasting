...
- Recognise the renewal equation as an epidemiologically-informed autoregression
- Add mechanism to a forecast: generation time, delays, and susceptible depletion
- Backtest models with more and less mechanism and score them
- See what a misspecified mechanism does to a forecast
...
- Normal observation error, scale $\sigma$
:::
...
Both are an observation drawn from a distribution around a weighted sum of past values; the renewal fixes the weights to $R_t$ times the generation time, and can add imports $\iota_t$, which we set to zero.
