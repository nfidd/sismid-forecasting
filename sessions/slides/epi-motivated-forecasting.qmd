...
- Observation error comes from the sampling distribution
- $\iota_t$ is imported infections, set to zero here
:::

::::

Same autoregressive shape, but the coefficients are $R_t$ times generation-time weights, and the observation error comes from the sampling distribution rather than an additive $\varepsilon_t$.
