...
format:
  revealjs:
    output: slides/time-series-101.html
    footer: "Time Series Forecasting 101"
    chalkboard: true
    slide-level: 3
---
...
### Using Data to Inform Modeling Choices
...
```{r}
pacf_plot <- ggtime::gg_tsdisplay(flu_data, y = wili, plot_type='partial', lag_max = 104)
pacf_plot
```
...
::: {.callout-important title="Implications in practice"}
...
### A Lag-1 model: ARIMA(p=1,d=0,q=0)

Here is the equation for an ARIMA(1,0,0) model: 
$$ y_t = \mu_t + \phi_1 y_{t-1} + \varepsilon_t $$ 

 - $\mu_t$ is a constant (like an intercept term in the regression)
 - $\phi_1$ is a coefficient for the auto-regressive term
 - $\varepsilon_t$ is a random error term with mean zero and constant variance

::: {.callout-warning}
Note the constant variance assumption!
:::

### A Random Walk model: ARIMA(p=0,d=1,q=0)

Here is the equation for an ARIMA(0,1,0) model: 
...
::: {.callout-note}
This model is the same as the last slide but here both the "outcome" variable and predictor variable are now first-order differences, corresponding to the second "1" in (1,1,0).
:::
...
[Return to the session](../time-series-101.qmd)
