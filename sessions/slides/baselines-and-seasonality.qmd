...
data(flu_data)
flu_data <- flu_data |>
  filter(epiweek <= "2017-08-27") # start of the 2017/2018 season
...
- Transparent, cheap, hard to overfit
- A *shared* baseline lets everyone measure "skill" on the same yardstick

### Three simple baselines

- **Persistence** — "nothing changes": the most recent observation
- **Mean** — "it reverts to normal": the long-run average
- **Seasonal** — "this time of year looks like it usually does": the typical seasonal pattern
...
Persistence extends the last value; the mean ignores where we are. Honest, but beatable.
...
The combined model captures the seasonal shape *and* starts from the right level — but "looks better" isn't proof. That's next session.
...
fit_ar2f |> gg_tsresiduals()
```

## `r fontawesome::fa("laptop-code", "white")` Your Turn {background-color="#447099" transition="fade-in"}

1.  Build the three baselines: `NAIVE()`, `MEAN()`, and a `fourier()` seasonal model.
2.  Add recent dynamics with an AR + `fourier()` model and compare against the baselines.
