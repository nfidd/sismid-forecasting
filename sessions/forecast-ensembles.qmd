...
As we saw in the sessions on [baselines and seasonality](baselines-and-seasonality.qmd), [forecast evaluation](forecast-evaluation.qmd), and [real-world forecasts](real-world-forecasts.qmd), different modelling approaches have different strength and weaknesses, and we do not usually know in advance which one will produce the best forecast in any given situation.
...
-   [Introduction to ensembles](slides/introduction-to-ensembles.qmd)
...
In this session we will use the `fable` package for fitting simple forecasting models, the `dplyr` package for data wrangling, the `ggplot2` and `ggtime` packages for plotting and the `epidatr` package for accessing and downloading versions of epidemiological surveillance data from the [Delphi EpiData API](https://cmu-delphi.github.io/delphi-epidata/).
...
Probabilistic predictions can be described as coming from probability distributions.
...
-   Another approach uses values that corresponds to a specific set of **quantile** levels of the predictive distribution.
...
![Table 3 from [@shandross_multi-model_2026]](slides/figures/ensemble-approach-table.png)
...
Our forecast models use the `my_fourth_root` transformation, which the course package provides.

# Individual forecast models

In this session we will use the forecasts from the models we explored in the session on [forecast evaluation](forecast-evaluation.qmd).
These were:
...
As in the session on [forecast evaluation](forecast-evaluation.qmd), we will fit these models to a range of forecast dates.
...
In this section, we will start using hubverse-style data for forecast output formats, as we find it to be a flexible, easily extensible, and general set-up for data formats that will enable activities later in the course.
...
Now that we have standardized our data formatting, we can create forecasts as a combination of multiple forecasts.
...
As in the [forecast evaluation session](forecast-evaluation.qmd), we can evaluate the accuracy of the ensembles.
Here, instead of using the `fable` utilities, we will start to use the hubverse [`hubEvals` package](https://hubverse-org.github.io/hubEvals/) which relies on [the `scoringutils` package](https://epiforecasts.io/scoringutils/index.html).
...
filtered_forecasts <- quantile_forecasts |>
  filter(!(model_id %in% c("rw", "fourier")))
```
