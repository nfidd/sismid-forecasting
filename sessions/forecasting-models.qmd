...
In this session we will use `nfidd` package for some flu data, the `fable` package to fit and forecast ARIMA models, the `dplyr` and `tidyr` packages for data wrangling, the `ggplot2` and `ggtime` packages for plotting (`ggtime` provides the time-series and forecast plotting functions, such as `gg_tsdisplay()`, that were previously part of `fabletools`/`feasts`), and the `epidatr` package for loading epidemiological data.
