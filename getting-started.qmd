...
NFIDD (Nowcasting and Forecasting Infectious Disease Dynamics) is an MIT-licensed library of [sessions](reference/sessions) for learning about nowcasting and forecasting of infectious disease surveillance data.
This is the **Forecasting & evaluation** course, covering the second half of the SISMID week.
A companion [Nowcasting & $R_t$ estimation course](https://nfidd.github.io/sismid-nowcasting/) covers the first half.
...
2.  Install the `nfidd.forecasting` R package
3.  Download course materials (if using the hybrid approach you may not need to do this)
...
#### Installation of the `nfidd.forecasting` package

To install the packages needed in the course, including the `nfidd.forecasting` package that contains the data files used throughout:

```{r install, eval=FALSE}
if (!require("pak")) {
    install.packages("pak")
}
pak::pak("nfidd/sismid-forecasting", dependencies = TRUE)
```
...
remotes::install_github("nfidd/sismid-forecasting", dependencies = TRUE)
```
:::
...
-   Directly download the course material:
    ...
    ::: callout-tip
    [[**Download**]{.underline}](https://github.com/nfidd/sismid-forecasting/archive/refs/heads/main.zip)
    :::

-   Alternatively, if you are familiar with git you can clone the [repo](https://github.com/nfidd/sismid-forecasting).
...
1.  **Update the nfidd.forecasting package** (this is quick and ensures you have the latest functions):
...
2.  **Download fresh course materials** if using local files:
    ...
    -   Download the latest version: [[**Download**]{.underline}](https://github.com/nfidd/sismid-forecasting/archive/refs/heads/main.zip)
